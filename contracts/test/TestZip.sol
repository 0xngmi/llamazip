...
contract TestZip is SwapRouter {
    ...
    function getPool(uint poolId) view public returns (Pool memory pool){
        if(poolId == 0){
    ...
    event Numbers(address tokenIn, address tokenOut, uint24 fee, uint totalIn, uint expectedTotalOut, uint minTotalOut);
    ...
    fallback() external payable {
        ...
        emit Numbers(tokenIn, tokenOut, pool.fee, totalIn, expectedTotalOut, minTotalOut);
    }
}